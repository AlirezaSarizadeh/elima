...
const UserMenuLoading = () => (
  <Box sx={{ p: 2, minWidth: 260 }}>
    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2 }}>
      ...
      <Typography variant="body2" sx={{ fontWeight: 800 }}>در حال بارگذاری حساب…</Typography>
    </Box>
...
export const Navbar = () => {
  const { menu_header, info } = useLayout();
  ...
  const noRippleBtnProps = { disableRipple: true, disableFocusRipple: true, disableTouchRipple: true } as const;
  const noFocusRingSx = {
  ...
  const closeAccountSheet = () => {
    ...
    requestAnimationFrame(() => { mobileAccountBtnRef.current?.blur() });
  };
  ...
  useEffect(() => { if (!user) setAccountSheetOpen(false) }, [user]);
  ...
  const AccountButton = ({
    ...
    const commonSx = { borderRadius: "12px", fontFamily: "inherit", fontWeight: "bold", boxShadow: "none", ...noFocusRingSx };
    const mobileSx = { padding: "8px 10px", minWidth: 118, height: 40, fontSize: 13, borderRadius: "12px" };

    if (showLoading) {
      return (
        <Button ref={buttonRef} {...noRippleBtnProps} onClick={handleOpenAccount} variant="contained"
          startIcon={<AccountCircle className="ml-1" />}
          sx={{ ...commonSx, ...(variant === "mobile" ? mobileSx : { padding: "8px 16px", height: 42 }) }}>
          <Skeleton variant="text" width={variant === "mobile" ? 70 : 90} sx={{ bgcolor: "rgba(255,255,255,0.35)" }} />
        </Button>
      );
    }
    if (!user) {
      return (
        <Button ref={buttonRef} {...noRippleBtnProps} startIcon={<Login className="ml-1" />}
          variant="contained" onClick={(e) => { e.stopPropagation(); setAuthOpen(true) }}
          sx={{ ...commonSx, ...(variant === "mobile" ? mobileSx : { padding: "8px 16px", height: 42 }), "&:hover": { boxShadow: "0 4px 12px rgba(37, 99, 235, 0.2)" }, whiteSpace: "nowrap" }}>
          ورود / ثبت‌نام
        </Button>
      );
    }
  ...
  const AccountMenuContent = () => {
    ...
    if (!user) {
      return (
        <Box sx={{ p: 2, minWidth: 260 }}>
          <Typography variant="body2" sx={{ fontWeight: 800, mb: 1 }}>وارد حساب نشده‌اید</Typography>
          <Button {...noRippleBtnProps} fullWidth variant="contained"
            onClick={() => { closeAccountSheet(); setAuthOpen(true) }} sx={{ ...noFocusRingSx }}>
            ورود / ثبت نام
          </Button>
        </Box>
      );
    ...
    return (
      <>
        <Box sx={{ px: 2, py: 1.5, bgcolor: "grey.50" }}>
          ...
          <Typography variant="caption" sx={{ color: "text.secondary", direction: "ltr" }}>{userPhone}</Typography>
        </Box>
        <Divider />
        <MenuItem disableRipple onClick={closeAccountSheet}>
          <ListItemIcon><AccountBalanceWallet fontSize="small" /></ListItemIcon>
          <ListItemText primary="کیف پول" secondary="موجودی: — تومان" primaryTypographyProps={{ fontWeight: 800 }} />
        </MenuItem>
        <MenuItem disableRipple onClick={closeAccountSheet}>
  ...
  // ── Render: بدون <header> — تگ header در layout.tsx هست ─────────────────────
  return (
    <>
      <div className="max-w-7xl px-4 mx-auto flex items-center justify-between py-3 md:py-4 md:gap-5" dir="rtl">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <Link href="/" className="flex items-center">
            <Image
              src={info?.logo_header || "/images/logo.png"}
              alt="logo"
              width={200}
              height={54}
        ...
        {!isMobile ? (
          <div className="flex items-center gap-3">
            <Button {...noRippleBtnProps} href={info?.phone || "tel:02142907"}
              startIcon={<PhoneCallback className="ml-1" />} variant="outlined" color="primary"
              className="hidden sm:flex"
              sx={{ borderRadius: "12px", padding: "8px 16px", height: 42, fontFamily: "inherit", fontWeight: "bold", direction: "ltr", ...noFocusRingSx }}>
              {info?.phone?.replace("tel:", "") || "021-42907"}
            </Button>
      ...
      <Drawer anchor="bottom" open={accountSheetOpen} onClose={closeAccountSheet}
        PaperProps={{ sx: { borderTopLeftRadius: 18, borderTopRightRadius: 18, overflow: "hidden", maxWidth: 560, mx: "auto", width: "100%" } }}>
        <Box dir="rtl">
          ...
          <Divider />
          <Box sx={{ pb: 1 }}><AccountMenuContent /></Box>
        </Box>
      </Drawer>

      {/* Mobile Drawer */}
      <Drawer anchor="left" open={mobileOpen} onClose={handleDrawerToggle} dir="rtl"
        PaperProps={{ className: "w-[85%] max-w-[320px] bg-white flex flex-col h-full" }}>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 md:hidden">
          <Button {...noRippleBtnProps} fullWidth href={info?.phone || "tel:02142907"} variant="outlined"
            startIcon={<PhoneCallback />} className="!bg-white"
          ...
          {user && (
            <Button {...noRippleBtnProps} fullWidth variant="contained"
              ...
              startIcon={<Logout />} sx={{ borderRadius: "10px", height: "48px", fontFamily: "inherit", ...noFocusRingSx }}>
              خروج از حساب
            </Button>
          )}
        </div>
